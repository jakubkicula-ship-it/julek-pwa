// ./js/parent.js
// Logika panelu RODZICA
// Decyzje dla "wniosków o punkty" są wyłącznie w app.js
...
(function(){
  ...
  /* ================== USUWANIE WPISU (TYLKO DZIŚ KALENDARZOWO) ================== */
  ...
  window.renderParentPointRequests = function(){
    ...
    sorted.forEach(o=>{
      ...
      if(lockedAuto || blockedByOther || expd){
        ...
        ok.onclick = async ()=>{
          const res = await window.acceptPointRequest?.(o);
          if(res && res.ok === false && res.reason === "duplicate"){
            alert("Ten wniosek został rozpoznany jako duplikat i nie naliczono punktu.");
          }
        };
        ...
        no.onclick = async ()=>{
          ...
          await window.rejectPointRequest?.(o, comment.trim());
        };
      } else {
        ok.disabled = true;
        no.disabled = true;
